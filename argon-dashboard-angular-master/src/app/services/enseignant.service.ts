...
export class EnseignantService {
 ...
 getAllTeacherCount : string="http://localhost:3000/api/enseignant/teacherCount";
...
private getById : string="http://localhost:3000/api/enseignant";
  ...
  constructor(private httpClient: HttpClient,private userService : UserService ) {  this._loginObservable= new  BehaviorSubject({}) ; }
...
private saveTokenToLOcalStorage(token : string){
  ...
  saveEnseignant(data : any){
    ...
    return this.httpClient.post(this.enseignantUrl, data, { headers })
    .pipe(
      map((result: {  enseigant: Enseignant }) => {
        return <Enseignant>result.enseigant
  ...
  updateEnseignant(data, id) {
    ...
    return this.httpClient.patch(`${this.enseignantUrl}/${id}`, data, { headers })
 }

  getEnseignantById(id ){
    return this.httpClient.get<Enseignant>(`${this.getById}/${id}`).pipe(
  ...
  login(caredentials : {email :string , password : string}){
    return this.httpClient.post(this.loginUrl,caredentials)
    .pipe(
  ...
  getProfile(){
    ...
    return this.httpClient.get(`${this.profileUrl}/${id}`,{headers})
    .pipe(
      map((result )=>{
       return result
      })
  ...
  getTeacherCount(){
    ...
    return this.httpClient.get(this.getAllTeacherCount,{headers})
    .pipe(
      map((result )=>{
       return result
      })
